...
## Owner teach-back before completion

The owner must be able to explain:

- why raw input and Brief revisions are separate;
- who may transition each state;
- what happens when proposal generation fails after capture;
- how duplicate create and approval requests behave;
- why the fake is implemented before the live model;
- which tests prove the approval and revision invariants.
